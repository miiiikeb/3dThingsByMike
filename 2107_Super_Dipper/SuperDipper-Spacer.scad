...
module rackNeg(){
    for(xOS = [-(countX)*headOS/2:headOS:(countX)*headOS/2]){
        translate([xOS,0,0]){
            rotate([90,0,0]) cylinder(r=headOS/2-0.3 + space,h=discH + 2 * space,center = true,$fn = 120);
            screw(dims = screwDimsLong, os = [0,14,0],rot = [90,0,0]);
            qCube(dims = [2 * (headOS/2-0.3 + space),discH + 2 * space,headOS/2-0.3 + space]);
            rotate([-90,0,0]) cylinder(r=headOS/2 - arrowHoleR + space,h=50,center = false,$fn = 120);
            qCube(dims = [2 * (headOS/2 - arrowHoleR + space),50,headOS/2-0.3 + space],os = [0,25,0]);
            for(a = [-30,-150]) {
                rotate([0,a,0]) translate([8,0,0]) rotate([90,0,0]) qCyl(rad = 2.7/2 + space,hei = 15,res = res,os = [0,0,0]);
            }
            difference(){
                qCyl(rad = 8 + 2.7/2 + space,hei = 15,res = res,os = [0,0,0],rot = [90,0,0]);
                qCyl(rad = 8 - (2.7/2 + space),hei = 15,res = res,os = [0,0,0],rot = [90,0,0]);
                qCube(dims = [big,big,big],rot = [0,150,0]);
                qCube(dims = [big,big,big],rot = [0,-150,0]);
...
difference(){
    hull(){
        for(xOS = [-(countX)*headOS/2,(countX)*headOS/2]){
            translate([xOS,0,0]) rotate([0,0,0]) cylinder(r=headOS/2-0.3 + space,h=4,center = true,$fn = 120);
        }
    }
    #translate([0,0,5]) rotate([90,0,0]) rackNeg();
}
...
module spacerPos(){
    rotate([90,0,0]) cylinder(r=headOS/2-0.3,h=countY * headOS + 2 * (topTaperR + skin),center = true,$fn = 120);
    
    difference(){
        translate([0,countY * headOS/2 + (topTaperR + skin),0]) rotate([-90,0,0]) cylinder(r=headOS/2-1.5,h=discH,center = false,$fn = 120);
        translate([0,0,-(big + spaceZ)/2-axOS]) cube(size = big, center = true);
    }
}
...
module spacerNeg(){
    difference(){
        union(){
            translate([0,0,-axOS]) for(zOS = [-(big + spaceZ)/2,(big + spaceZ)/2]){
                translate([0,0,zOS]) cube(size = [big,countY * headOS + 2 * (topTaperR + skin),big], center = true);
            }
            translate([0,countY * headOS/2 + (topTaperR + skin),0]) rotate([90,0,0]) difference(){
                cylinder(r=headOS,h=discH,center = false,$fn = 120);
                cylinder(r=headOS/2 - arrowHoleR ,h=discH,center = false,$fn = 120);
            }   
        }
        hull(){
            translate([0,0,spaceZ/2-lil]) cube(size = [discH/2,(countY * headOS  + 2 * (topTaperR + skin)),lil], center = true);
             
            translate([0,(countY * headOS /2 + (topTaperR + skin)),spaceZ/2 + (headOS/2 - arrowHoleR - discH - 1)]) cube(size = [discH/2,lil,lil], center = true);
        }
        translate([0,0,0]) rotate([90,0,0]) cylinder(r=spineR,h=countY * headOS + 2 * (topTaperR + skin),center = true,$fn = 120);
    }
    translate([0,0,-axOS]) for(xOS = [-headOS/2,headOS/2]){
        for(yOS = [-(countY-1)*headOS/2:headOS:(countY)*headOS/2]){
            translate([xOS,yOS,0]){
               translate([0,0,-spaceZ/2]) cylinder(r = arrowHoleR, h = skin,center = false, $fn = 30);
               translate([0,0,-spaceZ/2 + skin]) cylinder(r1 = arrowHoleR, r2 = arrowHoleR + taperDelta, h = spaceZ - skin,center = false, $fn = 30);
            }
        }
    }
    ...
    // Axial Screws
    for(a = [-90,90]){
        translate([0,0,0]) rotate([a,0,0]) qCyl(rad = 2.7/2,hei = 35,res = res,os = [0,0,countY * headOS /2 + (topTaperR + skin) - 30]);
    }
    
    
    rotate([0,-135,0]) translate([8,0,0]) rotate([-90,0,0]){
        qCyl(rad = 2.7/2,hei = 35,res = res,os = [0,0,countY * headOS /2 + (topTaperR + skin) - 15]);
        nut(nut_dia=flangeNutD, nut_height = flangeNutH, os = [0,0,countY * headOS /2 + (topTaperR + skin) - 17], rot = [0,0,45]);
    }
    
    rotate([0,-45,0]) translate([8,0,0]) rotate([-90,0,0]){
        qCyl(rad = 2.7/2,hei = 35,res = res,os = [0,0,countY * headOS /2 + (topTaperR + skin) - 15]);
        nut(nut_dia=flangeNutD, nut_height = flangeNutH, os = [0,0,countY * headOS /2 + (topTaperR + skin) - 17], rot = [0,0,15]);
    }
}
...
hoseOD = 22.7;
hoseCone = [15,2];  //[length,skin]
module couplerHose(){
    difference(){
        hull() translate([0,(countY-1) * headOS/2 + couplerDims[1]/2 + pipeDia/2 + 0.8 * dSkin + couplerDims[1],0]){
            qCyl(rad=couplerDims[0]/2,hei=hoseCone[1],os=[0,0,couplerDims[2]],rot=[90,0,0]);
            qCyl(rad=hoseOD/2 + hoseCone[1],hei=hoseCone[0],os=[0,0,couplerDims[2]+1],rot=[-90,0,0]);
        }
        xrange = (countX-1)*headOS/2;
        xgrad = xfall/xrange;
        yrange = (countY-1)*headOS;
        ygrad = yfall/yrange;
        translate([0,(countY-1) * headOS/2 + couplerDims[1]/2 + pipeDia/2 + 0.8 * dSkin + couplerDims[1],0]){
            #qCyl(rad=hoseOD/2,hei=1000,res = res,os=[0,-50,couplerDims[2]+1],rot=[-90,0,0]);
        }
        nutNbolt(os = [0,(countY-1) * headOS/2 + couplerDims[1] + pipeDia/2 + 0.8 * dSkin,20], rot = [0,0,0]);
        for(a = [-50,0,50]){
            translate([0,0,couplerDims[2]]) rotate([0,a,0]) nutNbolt(os = [0,(countY-1) * headOS/2 + couplerDims[1] + pipeDia/2 + 0.8 * dSkin,couplerDims[0]/2-5], rot = [0,0,0]);
...
plugConeZ = 8;
module plug(){
    qCyl(rad=hoseOD/2 + 2,hei=4,os=[0,0,0],rot=[0,0,0],res = 1);
    qCyl(rad=hoseID/2,hei=plugZ,os=[0,0,4],rot=[0,0,0],res = 1);
    qCone(rad1=hoseID/2,rad2=hoseID/2 - 2,hei=plugConeZ,os=[0,0,plugZ + 4],rot=[0,0,0],res = 1);
}
